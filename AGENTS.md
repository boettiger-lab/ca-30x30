...
|---|---|---|
| **Conifer Woodland / pinyon-juniper — SOLVED 2026-08-02, and it was our defect.** The FVEG-derived `cwhr`/`cwhr13` assets are **not clipped to California**: 19.4% of pinyon-juniper (335,544 ac) is in Nevada, where the CA conserved-areas layer cannot reach, so it all scored non-conserved. CA-clipped, PJ is **52.0%** vs report 52.6%. The same clip fixes sagebrush (19.9→22.4), low sage (11.6→13.8), bitterbrush (12.1→13.6 exact), conifer woodland (28.8→32.0) and shrub (26.6→27.6) — all Great Basin classes. Juniper reproduced all along *because* it is 99.9% in-state; treating it as a passing control hid the extent bug for weeks. **Before attributing any divergence to vintage or a partner's layer, check that our data is inside California.** Handled in two layers: the asset stays faithful to the source footprint and STAC documents the true extent (data-workflows#505), while `system-prompt.md` carries the app rule to mask features to California (via `ca30x30-ecoregion` hex on `h8`, the same CA definition as the pinned denominator). | App rule shipped; STAC note filed | data-workflows#505, ca-30x30#103 |
| **The "`cwhr13` is missing ~5M acres" claim is FALSE — do not re-investigate.** Measured: the res-10 cell union is 102,834,000 ac (mode and fractional builds identical) vs California's 101,498,000 — **1.3% larger**, from boundary cells overhanging the state edge. `whr13num=0` nodata is 91,371 ac, not 5M. The ~96.5M figure in the `cwhr13-hex-fractions` STAC note is cell count x H3's **global average** res-10 area (0.0150475 km²); California's cells run ~6.6% larger (0.016040 km²), so the global constant understates by ~5%. Per-class percentages are unaffected (a uniform area scale cancels in a ratio). Agents quote that note to users, and one derived a bogus "3.718 ac/cell" from it. Note the 1.3% excess is **not** only edge overhang — ~1.4M ac of it is genuinely out-of-state (data-workflows#505). | Note fix open; data is correct | [data-workflows#504](https://github.com/boettiger-lab/data-workflows/issues/504) (ca-30x30#73 closed) |
| **Wetlands — reconciled, do not re-open.** The app feature is three NWI `WETLAND_TYPE` classes = **1,793,850 ac** vs the assessment's 1,735,518 (**+3.4%**, NWI vintage). The often-quoted "our CA total is 9.14M, nothing reconciles" compared their *feature* against *all eight* NWI types, including Deepwater/Lake/Riverine which are not in it. | Settled | ca-30x30#105 (closed) |
...
| **Streams / stream order — SOLVED 2026-08-08. Route to `usgs-nhdplus-hr-flowline` and mask it to California.** Base NHD's `STREAMORDER` is usable on 8.7% of California, 97.8% of it HUC4 1801 (Klamath), so any order answer off that asset is one basin mislabelled as the state — sparsity in USGS's own `NHDFlowlineVAA`, **not our import; do not re-run the ingest.** NHDPlus HR replaces it for order and network attributes only (`streamorde`, 100.00% of in-network non-coastline length in all 266 units) and sits **alongside** base NHD, which keeps extent and flow permanence: base NHD is denser and holds ~289,000 km of ephemeral channel NHDPlus HR lacks, and their `lengthkm` differ +1.9–5.5% per editing vintage, so never add lengths across the two. **The new collection is national and clipped to nothing** — unmasked, California headwaters read 924,634 km against 641,393 km in-state, and the conserved share 19.0% against 27.5%; mask on `h8`+`h0` against `ca30x30-ecoregion` exactly as for pinyon-juniper. Four things were measured and ruled out — **do not re-investigate**: hex-on-linestring (≤1 pp on GAP 1+2 vs exact `ST_Intersection`), proportional-vs-categorical GAP weighting, `FCode` redefinition between NHD products (~96% stable), and the "H3 can only measure area" premise (false — weight by deduped `lengthkm` and use the hex only for the per-cell GAP fraction). The partner is on a medium-resolution network, so our lengths run ~3× hers permanently — class proportions and GAP 1+2 agree, GAP 3+4 will not (scale-sensitive: 20% on mainstems, 46% on tributaries). | App rule shipped (`system-prompt.md`); upstream closed | [ca-30x30#111](https://github.com/boettiger-lab/ca-30x30/issues/111) (closed), [data-workflows#518](https://github.com/boettiger-lab/data-workflows/issues/518), [#205](https://github.com/boettiger-lab/data-workflows/issues/205), [#528](https://github.com/boettiger-lab/data-workflows/issues/528) (all closed) |
| **Streams — bucket listing** — `public-usgs-nhd` denies `ListBucket`, so glob reads 403. Workaround: enumerate `h0` explicitly. | Fixed | [data-workflows#411](https://github.com/boettiger-lab/data-workflows/issues/411) (closed) |
