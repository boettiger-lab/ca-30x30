...
| **Streams / NHD stream order — diagnosed, do not re-derive.** Usable `STREAMORDER` covers 8.7% of California and 97.8% of that is HUC4 1801 (Klamath), so any stream-order answer is one basin mislabelled as the state. That is sparsity in USGS's own `NHDFlowlineVAA` — **not our import; do not re-run the ingest.** NHDPlus HR is verified to fix it (99.65% coverage across all 13 CA basins) and is being added alongside, not instead of, the current asset. Three things were measured and ruled out — **do not re-investigate**: hex-on-linestring (≤1 pp on GAP 1+2 vs exact `ST_Intersection`), proportional-vs-categorical GAP weighting (no difference), and any `FCode` redefinition between NHD products (~95% stable). The partner is on a medium-resolution network, so high-res lengths will not match hers even after the fix — GAP 1+2 should still agree, GAP 3+4 will not (it is scale-sensitive: 20% on mainstems, 46% on tributaries). Numbers, SQL and next actions live in the issues. | App rule pending; upstream in progress | [ca-30x30#111](https://github.com/boettiger-lab/ca-30x30/issues/111), [data-workflows#518](https://github.com/boettiger-lab/data-workflows/issues/518), [#205](https://github.com/boettiger-lab/data-workflows/issues/205) |
