...
| **Streams — take stream order from `usgs-nhdplus-hr-flowline` and mask it to California.** That collection is national and clipped to nothing; unmasked, California headwaters read 924,634 km against 641,393 km in-state and 19.0% conserved against 27.5%, so mask on `h8`+`h0` against `ca30x30-ecoregion` as for pinyon-juniper. It covers order and network attributes only — extent and flow permanence stay on base NHD, which is denser by ~289,000 km of ephemeral channel, so never add lengths across the two. Base NHD's own `STREAMORDER` is usable on 8.7% of California, 97.8% of it HUC4 1801 (Klamath): that is sparsity in USGS's `NHDFlowlineVAA`, **not our import — do not re-run the ingest.** **Do not re-investigate** hex-on-linestring (≤1 pp on GAP 1+2 vs exact `ST_Intersection`), proportional-vs-categorical GAP weighting, `FCode` redefinition between NHD products (~96% stable), or "H3 can only measure area" (false — weight by deduped `lengthkm`, hex only for the per-cell GAP fraction). Our lengths run ~3× the partner's medium-resolution network permanently: class proportions and GAP 1+2 agree, GAP 3+4 will not (20% on mainstems, 46% on tributaries). | App rule shipped (`system-prompt.md`); upstream closed | [ca-30x30#111](https://github.com/boettiger-lab/ca-30x30/issues/111), [data-workflows#518](https://github.com/boettiger-lab/data-workflows/issues/518), [#205](https://github.com/boettiger-lab/data-workflows/issues/205), [#528](https://github.com/boettiger-lab/data-workflows/issues/528) (all closed) |
