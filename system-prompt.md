...
- **A cell's conserved share is a fraction, not a category.** For per-cell GAP fractions use the conserved-areas `hex-weights` assets (res-10, plus `-res9` and `-res8` rollups) — pick the one matching the feature's native resolution, and follow the aggregation rules in their `get_schema` description rather than rolling your own `GROUP BY` on the hex asset. This app's grouping: **GAP 1+2 = `w1+w2`** is the 30x30-conserved share, **GAP 3+4 = `w3+w4`** is "other conserved or public lands", and the remainder is land outside any conserved-area unit. A feature's share in each category is `SUM(frac × w) / SUM(frac)` over the feature's cells, with `frac = 1` for presence-only features. Never reduce a cell to conserved/not-conserved — on a feature grid coarser than res-10 that inflated ACE BioRank-5 from 21% to 33% and cut its non-conserved share from 50% to 30%. If the user asks for GAP 3 and GAP 4 separately, report them separately — GAP 4 is not a conservation category (see above).
